...
with DAG(
    ...
    def download_file(url, **kwargs):
        ...
        try:
            ...
            task_instance.xcom_push(key='nb_rows', value=nb_rows) # Push number of rows as 'nb_rows'
            task_instance.xcom_push(key='file_name', value=tmp_file) # Push filename as 'file_name'
            task_instance.xcom_push(key='tmp_file_path', value=tmp_file_path) # Push file path as 'tmp_file_path'
    ...
    upload_data_to_gcs = LocalFilesystemToGCSOperator(
        ...
        bucket=f"{GCP_PROJECT}-bucket-dev",
    )
    ...
    gcs_to_bq = GoogleCloudStorageToBigQueryOperator(
        ...
        destination_project_dataset_table="week1_350310_dataset_dev.raw_table",
        source_format="CSV",
        autodetect=True,
        skip_leading_rows=1,
    ...
    dbt_test = BashOperator(
        task_id='dbt_test',
        bash_command=f"dbt test --profiles-dir {DBT_FOLDER} --project-dir {DBT_FOLDER} --vars '{{nb_rows: "+"{{ ti.xcom_pull(task_ids='download', key='nb_rows') }}"+"}'",
        dag=dag
    )
